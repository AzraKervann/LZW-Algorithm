...
class DiffLZW:
    ...
    def to_diff(self, arr):
        rows, cols = arr.shape
        delta = np.zeros((rows, cols), dtype=np.int32)
        delta[0, 0] = arr[0, 0]

        for r in range(1, rows):
            delta[r, 0] = int(arr[r, 0]) - int(arr[r - 1, 0])

        for r in range(rows):
            for c in range(1, cols):
                delta[r, c] = int(arr[r, c]) - int(arr[r, c - 1])
    ...
    def compress(self):
        ...
        arr = np.array(img)
        shape = arr.shape
        diff = self.to_diff(arr)

        entropy = self.compute_entropy(diff)
        codes = self._encode(diff)
        bits = self._codes_to_bits(codes)

        header = f"{shape[0]},{shape[1]}|"
        ...
        out_path = self.filepath + "_lv3_compressed.bin"
    ...
    def decompress(self, bin_path):
        ...
        arr = self.from_diff(flat, (rows, cols))
        Image.fromarray(arr, "L").save(
            out_path := bin_path.replace("_lv3_compressed.bin", "_lv3_decompressed.png")
        )
        return out_path